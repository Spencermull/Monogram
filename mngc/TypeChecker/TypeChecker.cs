...
public class TypeChecker
{
    ...
    private MgType ResolveTypeNode(TypeNode type) => type switch
    {
        ...
        NamedTypeNode { Name: "transmutex" } => new MgStruct("transmutex"),
        NamedTypeNode { Name: "sink" }        => new MgStruct("sink"),
        NamedTypeNode { Name: "bucket" }      => new MgStruct("bucket"),
        NamedTypeNode { Name: "pool" }        => new MgStruct("pool"),
        NamedTypeNode { Name: "graph" }       => new MgStruct("graph"),
        NamedTypeNode { Name: "poly" }        => new MgStruct("poly"),
        NamedTypeNode { Name: "pod" }         => new MgStruct("pod"),
        NamedTypeNode { Name: "utdctrl" }     => new MgStruct("utdctrl"),
        NamedTypeNode { Name: "argus" }       => new MgStruct("argus"),
        NamedTypeNode { Name: "bench" }       => new MgStruct("bench"),
        NamedTypeNode { Name: "kiln" }        => new MgStruct("kiln"),
    ...
    {
        // mono.sync
        "mono.sync.transmutex" => new MgStruct("transmutex"),
        "mono.sync.acquire"    => MgTypes.Void,
        "mono.sync.release"    => MgTypes.Void,
        "mono.sync.free"       => MgTypes.Void,
        // mono.pipe
        "mono.pipe.sink"       => new MgStruct("sink"),
        "mono.pipe.write"      => MgTypes.Void,
        "mono.pipe.sink_free"  => MgTypes.Void,
        "mono.pipe.bucket"     => new MgStruct("bucket"),
        "mono.pipe.fill"       => MgTypes.Void,
        "mono.pipe.drain"      => new MgArray(MgTypes.Void),
        "mono.pipe.bucket_free"=> MgTypes.Void,
        "mono.pipe.coagulate"  => new MgArray(MgTypes.Void),
        // mono.pool
        "mono.pool.new"        => new MgStruct("pool"),
        "mono.pool.alloc"      => new MgArray(MgTypes.Void),
        "mono.pool.reset"      => MgTypes.Void,
        "mono.pool.free"       => MgTypes.Void,
        // mono.linear
        "mono.linear.new"      => new MgStruct("linear"),
        "mono.linear.bind"     => MgTypes.Void,
        "mono.linear.run"      => new MgArray(MgTypes.Void),
        "mono.linear.free"     => MgTypes.Void,
        // mono.graph
        "mono.graph.new"       => new MgStruct("graph"),
        "mono.graph.add"       => MgTypes.Uint64,
        "mono.graph.link"      => MgTypes.Void,
        "mono.graph.unlink"    => MgTypes.Void,
        "mono.graph.has_edge"  => MgTypes.Int,
        "mono.graph.node"      => new MgArray(MgTypes.Void),
        "mono.graph.count"     => MgTypes.Uint64,
        "mono.graph.free"      => MgTypes.Void,
        // mono.inspect
        "mono.inspect.dump"    => MgTypes.Void,
        "mono.inspect.addr"    => MgTypes.Void,
        "mono.inspect.name"    => MgTypes.Void,
        "mono.inspect.int"     => MgTypes.Void,
        "mono.inspect.float"   => MgTypes.Void,
        // mono.glob
        "mono.glob.match"      => MgTypes.Int,
        "mono.glob.scan"       => MgTypes.Int64,
        // mono.utils
        "mono.utils.swap"      => MgTypes.Void,
        "mono.utils.ispow2"    => MgTypes.Int,
        "mono.utils.next_pow2" => MgTypes.Uint64,
        // mono.polymorph
        "mono.polymorph.new"   => new MgStruct("poly"),
        "mono.polymorph.bind"  => MgTypes.Void,
        "mono.polymorph.call"  => new MgArray(MgTypes.Void),
        "mono.polymorph.is"    => MgTypes.Int,
        "mono.polymorph.free"  => MgTypes.Void,
        // mono.podlib
        "mono.podlib.create"   => new MgStruct("pod"),
        "mono.podlib.attach"   => MgTypes.Void,
        "mono.podlib.run"      => new MgArray(MgTypes.Void),
        "mono.podlib.export"   => MgTypes.Int,
        "mono.podlib.import"   => new MgStruct("pod"),
        "mono.podlib.free"     => MgTypes.Void,
        // mono.utdctrl
        "mono.utdctrl.init"      => new MgStruct("utdctrl"),
        "mono.utdctrl.spawn"     => MgTypes.Void,
        "mono.utdctrl.telemetry" => MgTypes.Void,
        "mono.utdctrl.monitor"   => MgTypes.Void,
        "mono.utdctrl.shutdown"  => MgTypes.Void,
        // mtx.argus
        "mtx.argus.new"          => new MgStruct("argus"),
        "mtx.argus.log"          => MgTypes.Void,
        "mtx.argus.debug"        => MgTypes.Void,
        "mtx.argus.info"         => MgTypes.Void,
        "mtx.argus.warn"         => MgTypes.Void,
        "mtx.argus.error"        => MgTypes.Void,
        "mtx.argus.fatal"        => MgTypes.Void,
        "mtx.argus.free"         => MgTypes.Void,
        // mtx.benchmark
        "mtx.benchmark.new"      => new MgStruct("bench"),
        "mtx.benchmark.start"    => MgTypes.Void,
        "mtx.benchmark.stop"     => MgTypes.Void,
        "mtx.benchmark.report"   => MgTypes.Void,
        "mtx.benchmark.ms"       => MgTypes.Float64,
        "mtx.benchmark.run"      => MgTypes.Void,
        "mtx.benchmark.free"     => MgTypes.Void,
        // mtx.encode
        "mtx.encode.hex"         => new MgArray(MgTypes.Char),
        "mtx.encode.unhex"       => new MgArray(MgTypes.Void),
        "mtx.encode.base64"      => new MgArray(MgTypes.Char),
        "mtx.encode.utf8_valid"  => MgTypes.Int,
        "mtx.encode.ascii_only"  => MgTypes.Int,
        // mtx.hash
        "mtx.hash.fnv1a"         => MgTypes.Uint64,
        "mtx.hash.crc32"         => MgTypes.Uint32,
        "mtx.hash.djb2"          => MgTypes.Uint64,
        // mtx.compress
        "mtx.compress.encode"    => new MgArray(MgTypes.Byte),
        "mtx.compress.decode"    => new MgArray(MgTypes.Byte),
        // mtx.kiln
        "mtx.kiln.new"           => new MgStruct("kiln"),
        "mtx.kiln.stage"         => MgTypes.Void,
        "mtx.kiln.run"           => new MgArray(MgTypes.Void),
        "mtx.kiln.free"          => MgTypes.Void,
        // std.time
